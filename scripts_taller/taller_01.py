...
def photo():
    ...
    for i in range(count_photo):
        file_name = 'photo' + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.jpg'
        wait(3)
        speak('Sacando foto')
        setPictureSize("small")
        img = takePicture('grey')
        show(img)
        confirm = input('¿Quieres guardar la foto s/n o si/no?')
        if confirm == 's' or confirm == 'si':
            savePicture(img, file_name)
        print(file_name)
